...
Then,
\[
X''(s) + X(s) = (A \cos(\theta(s)) + B \sin(\theta(s))) \frac{J(X(s))}{\abs{X(s)}^2}
\]
for constants \(A, B\) and where \(J\) denotes the clockwise rotation by \(2\pi\). In particular \(X'' + X\) is pointwise orthogonal to \(X\).
\end{thm}
...
\[
DE_X(V) = -2\inpr{X'' + X}{V}_2.
\]
For convenience, let \(Y = X'' + X\) so that
\[
...
Therefore,
\[
X'' + X = Y = hN = -\frac{h}{f} J(X) = -\frac{fh}{f^2} J(X) = -(A \cos\theta + B \sin\theta) \frac{J(X)}{\abs{X}^2}.
\]
\end{proof}
