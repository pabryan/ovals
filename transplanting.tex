...
\section{The Constraint, \(\alpha\) is Natural for the Energy, \(E\)}

In this section we prove that the constraint, \(\alpha\) is a \emph{Natural Constraint} for the energy \(E\). That is, every constrained critical point of \(E\) is in fact an unconstrained critical point of \(E\).
...
\]
Then, \(X\) is an unconstrained critical point for \(E\), so that
\[
dE_X \equiv 0.
\]
\end{thm}
...
In particular, taking \(\bar{\rho} \equiv 0\) we have for every \(\bar{\mu}\),
\[
T = -\kappa\bar{\mu}\sin\theta e_1 + \kappa\bar{\mu}\cos\theta e_2
\]
and hence
\[
...
Thus we have that
\[
y_1 \perp_{L^2} \{\bar{\rho}(\cos\theta + \sin\theta\tan\theta\}.
\]

{\color{red} Now we want to show this implies $y_1 = 0$. I'm not sure whether this is true or not though! Maybe this shows how to find a counter example to the conjecture!}
\end{proof}
