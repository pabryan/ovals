...
&= -\frac{1}{E(X)} \int_{\So} \frac{1}{\abs{X}^2} \left(\inpr{X'}{X} \frac{X}{\abs{X}} + (A \cos\theta + B \sin\theta) N\right) \\
&= -\frac{1}{E(X)} \int_{\So} \frac{1}{\abs{X}^2} \left(\inpr{X'}{X} T + (A \cos\theta + B \sin\theta) N\right).
\end{split}
\]
If \(A, B = 0\) then we get \(X'' = -E(X) X\) and hence
\[
\alpha(X) = \frac{1}{E(X)} \int_{\So} \frac{E(X)X}{\abs{X}} ds = \int_{\So} \frac{X}{\abs{X}} ds = 0
\]
as expected. So that's nice and consistent.
...
If \(X\) is a minimiser subject to the constraint then by \Cref{thm:constrained_critical}, we have
\[
X''(s) + E(X) X(s) = (A \cos(\theta(s)) + B \sin(\theta(s))) \frac{J(X(s))}{\abs{X(s)}^2}
\]
for some constants \(A, B \in \RR\). We also have
\begin{equation}
\label{eq:hessian_minimiser_positive}
[\nabla^2 E(V, W)] (X) \geq 0
\end{equation}
for all \(V, W\) constraint preserving vector fields (i.e. vector fields tangent to the constraint sub-manifold \(\mathcal{C}\)) and where
\[
[d^2 E(V, W)] (X) = d[dE(V)]_X (W) - dE_X(\nabla_W V)
\]
...
\[
\nabla^2 E(V, W) = 2\left(\inpr{2\inpr{X}{V}_2 L(X) - L(V)}{W} + 2\inpr{X}{V}_2\right)
\]
